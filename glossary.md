...
1. **新增术语**：每次翻译完成后，新遇到的术语需要追加到此表中，保证跨书籍译名一致。

2. **译名原则**：
   - 优先参考经典中文译著（Craig《机器人学》、Thrun《概率机器人学》等）
   - 其次参考 IEEE RAS 官方中文资料
   - 新术语以"意译为主，音译为辅"原则处理

3. **专有名词**：人名、地名、算法名一般不翻译，保留原名：
   - ✅ `Denavit-Hartenberg` → D-H 参数法（保留原名缩写）
   - ✅ `Gazebo`、`ROS`、`MoveIt` → 直接用，首次附括号解释

4. **单位**：单位不翻译：`rad`、`m/s`、`N·m`、`Hz` 等保留原文
